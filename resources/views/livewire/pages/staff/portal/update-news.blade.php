...
usesFileUploads();
state(['newsId' => fn($id) => $id])->locked();
state(['categories' => [], 'tags' => [], 'published' => 'private']);
state(['tag', 'category_name', 'image', 'schedule', 'title', 'slug', 'content', 'image', 'currentImage']);

// Mount the data from the existing news
mount(function () {
    ...
    if ($news) {
        ...
        $this->published = $news->published_at ? 'published' : 'private';
        $this->currentImage = $news->image;
        $this->categories = CategoryInNews::where('news_id', $this->newsId)->pluck('news_category_id')->toArray();
        $this->tags = TagInNews::where('news_id', $this->newsId)->pluck('tag')->toArray();
    }
});
...
$currentCategories = computed(function () {
    return CategoryInNews::where('news_id', $this->newsId)->pluck('news_category_id')->toArray();
});
...
$post = function () {
    ...
    try {
        ...
        if ($news) {
            ...
            CategoryInNews::where('news_id', $news->id)->delete();
            ...
            TagInNews::where('news_id', $news->id)->delete();
        ...
        Toaster::error('Berita gagal diperbarui');
        Toaster::error($e->getMessage());
    }
};
...
<div>
    <x-ui.breadcrumbs :crumbs="[
        ...
        <x-slot:actions>
            <x-ui.button color="red" wire:click="delete" size="xs" wire:confirm="Anda yakin ingin menghapus data ini?">
                <x-slot:icon class="">
    ...
    <x-ui.modal id="add-category-modal">
        ...
        <x-slot name="content">
            <form wire:submit="addCategory" class="mx-auto">
                <x-ui.input id="name" name="name" wire:model="category_name" label="Nama Kategori" placeholder="Masukan Nama" main-class="mb-5"/>
                <div class="flex justify-end space-x-2">
                    <x-ui.button type="reset" color="light" wire:click="$dispatch('close-modal', {id: 'add-category-modal'})">
                        Batal
                    </x-ui.button>
                    <x-ui.button submit color="blue" wire:loading.attr="disabled" wire:loading.class="cursor-not-allowed" wire:target="addCategory">
                        Simpan
                    </x-ui.button>
                </div>
            </form>
                    ...
                    <div class=" mt-4">
                        ...
                        <div class="relative mt-3 rounded-xl overflow-hidden">
                            @if($this->currentImage)
                                <img class="object-cover" src="{{ asset('storage/' . $this->currentImage) }}" alt="Foto Utama" />
                                <div class="absolute top-0 right-0 p-1" data-popover-target="popover-left" data-popover-placement="left" wire:click="deleteImage" wire:confirm="Lanjutkan menghapus?">
                                    <div data-popover id="popover-left" role="tooltip" class="absolute z-10 invisible inline-block w-64 text-sm text-gray-500 transition-opacity duration-300 bg-white border border-gray-200 rounded-lg shadow-xs opacity-0 dark:text-gray-400 dark:border-gray-600 dark:bg-gray-800">
                                        ...
                                        <div class="px-3 py-2">
                                            <p class="">Tindakan ini akan menghapus gambar utama informasi/berita pada postingan.</p>
                                        </div>
